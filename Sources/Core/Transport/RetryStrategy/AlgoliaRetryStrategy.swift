...
public class AlgoliaRetryStrategy: RetryStrategy {
    ...
    public func retryableHosts(for callType: CallType) -> HostIterator {
        return queue.sync {
            // Reset expired hosts
    ...
    public func notify<T>(host: RetryableHost, result: Result<T, Swift.Error>) {
        return queue.sync {
    ...
    public func canRetry(inCaseOf error: Error) -> Bool {
