...
extension Rule.Edit: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        let type: EditType = try container.decode(Rule.Edit.EditType.self, forKey: .type)
        let delete: String = try container.decode(String.self, forKey: .delete)
        switch type {
            case .remove:
                self = .remove(delete)
            case .replace:
                let insert: String = try container.decode(String.self, forKey: .insert)
