...
extension InsightsEvent: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.type = try container.decode(InsightsEvent.EventType.self, forKey: .type)
        self.name = try container.decode(EventName.self, forKey: .name)
        self.indexName = try container.decode(IndexName.self, forKey: .indexName)
        self.userToken = try container.decodeIfPresent(UserToken.self, forKey: .userToken)
...
extension TimeInterval {
    var milliseconds: Int64 { Int64((self * 1000.0).rounded()) }
}
